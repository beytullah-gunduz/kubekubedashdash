...
object PrerequisiteChecker {
    ...
    private fun checkKubeconfig(): PrerequisiteCheck {
        // Every `$KUBECONFIG` entry counts, as it does for kubectl and for the
    ...
    // A plain parse of every kubeconfig entry: no exec plugin can run, so
    // nothing here can hang bootstrap (KubeconfigReader never throws either).
    private fun readContextNames(): List<String> = KubeconfigReader.Default.contextNames()
