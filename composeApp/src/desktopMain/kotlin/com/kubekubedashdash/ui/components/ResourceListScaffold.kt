 ...
 *  from a restart (F3), so pass the factory flow's — `{ restartListFlow(reactiveClient.services) }` —
 *  never the view model's derived state, which restartListFlow leaves alone. */
@Composable
